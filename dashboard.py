...
def is_valid_api_key_format(api_key):
    """Validate API key format for all supported providers"""
    import re
    
    # OpenAI keys
    if re.match(r'^sk-(admin-|proj-)?[a-zA-Z0-9]{20,}', api_key):
        return True
    # Anthropic keys
    if re.match(r'^sk-ant-[a-zA-Z0-9_-]+', api_key):
        return True
    # Brave Search
    if re.match(r'^BSA[a-zA-Z0-9]+', api_key):
        return True
    # NewsData.io
    if re.match(r'^pub_[a-zA-Z0-9]+', api_key):
        return True
    # Groq
    if re.match(r'^gsk_[a-zA-Z0-9]+', api_key):
        return True
    # 32-char hex (NewsAPI.org, GNews)
...
def fetch_newsdata_usage(api_key, days=30)::
    """Fetch usage data from NewsData.io API"""
    headers = {
    ...
    key_preview = f"{api_key[:15]}...{api_key[-15:]}" if len(api_key) > 30 else "SHORT_KEY"
    print(f"Testing NewsData.io key: {key_preview}")
    
    try:
        print(f"Testing NewsData.io API access...")
        test_response = requests.get(
        ...
        print(f"NewsData.io API response: {test_response.status_code}")
        if test_response.status_code == 200:
            print(f"✓ NewsData.io API access works!")
            result['status'] = 'success'
            result['basic_access'] = True
            result['message'] = 'NewsData.io API key is working'
            
            # Try to get usage info if available
            response_data = test_response.json()
            if 'totalResults' in response_data:
                result['sample_data'] = {
                    'total_results': response_data.get('totalResults', 0),
                    'articles_returned': len(response_data.get('results', []))
                }
        else:
            print(f"✗ NewsData.io API failed: {test_response.status_code}")
            result['status'] = 'error'
            result['error'] = f"NewsData.io API failed: {test_response.status_code} - {test_response.text}"
            result['error_details'] = get_error_details(test_response.text, test_response.status_code)
            
    except requests.exceptions.RequestException as e:
        print(f"✗ NewsData.io API network error: {str(e)}")
        result['status'] = 'error'
        result['error'] = f"NewsData.io API network error: {str(e)}"
        result['error_details'] = get_error_details(str(e), None)
    
    return result

def fetch_newsapi_org_usage(api_key, days=30):
    """Fetch usage data from NewsAPI.org"""
    headers = {
        'X-Api-Key': api_key
    }
    
    result = {'provider': 'newsapi_org'}
    key_preview = f"{api_key[:15]}...{api_key[-15:]}" if len(api_key) > 30 else "SHORT_KEY"
    print(f"Testing NewsAPI.org key: {key_preview}")
    
    try:
        print(f"Testing NewsAPI.org API access...")
        test_response = requests.get(
        ...
        print(f"NewsAPI.org API response: {test_response.status_code}")
        if test_response.status_code == 200:
            print(f"✓ NewsAPI.org API access works!")
            result['status'] = 'success'
            result['basic_access'] = True
            result['message'] = 'NewsAPI.org API key is working'
            
            response_data = test_response.json()
            if 'totalResults' in response_data:
                result['sample_data'] = {
                    'total_results': response_data.get('totalResults', 0),
                    'articles_returned': len(response_data.get('articles', []))
                }
        else:
            print(f"✗ NewsAPI.org API failed: {test_response.status_code}")
            result['status'] = 'error'
            result['error'] = f"NewsAPI.org API failed: {test_response.status_code} - {test_response.text}"
            result['error_details'] = get_error_details(test_response.text, test_response.status_code)
            
    except requests.exceptions.RequestException as e:
        print(f"✗ NewsAPI.org API network error: {str(e)}")
        result['status'] = 'error'
        result['error'] = f"NewsAPI.org API network error: {str(e)}"
        result['error_details'] = get_error_details(str(e), None)
    
    return result

def fetch_newsapi_ai_usage(api_key, days=30):
    """Fetch usage data from NewsAPI.ai (Event Registry)"""
    result = {'provider': 'newsapi_ai'}
    key_preview = f"{api_key[:15]}...{api_key[-15:]}" if len(api_key) > 30 else "SHORT_KEY"
    print(f"Testing NewsAPI.ai key: {key_preview}")
    
    try:
        print(f"Testing NewsAPI.ai API access...")
        # Corrected: Use eventregistry.org endpoint with proper parameters
        test_response = requests.get(
        ...
        print(f"NewsAPI.ai API response: {test_response.status_code}")
        if test_response.status_code == 200:
            print(f"✓ NewsAPI.ai API access works!")
            result['status'] = 'success'
            result['basic_access'] = True
            result['message'] = 'NewsAPI.ai API key is working'
            
            response_data = test_response.json()
            if 'articles' in response_data:
                result['sample_data'] = {
                    'articles_returned': len(response_data.get('articles', {}).get('results', []))
                }
        else:
            print(f"✗ NewsAPI.ai API failed: {test_response.status_code}")
            result['status'] = 'error'
            result['error'] = f"NewsAPI.ai API failed: {test_response.status_code} - {test_response.text}"
            result['error_details'] = get_error_details(test_response.text, test_response.status_code)
            
    except requests.exceptions.RequestException as e:
        print(f"✗ NewsAPI.ai API network error: {str(e)}")
        result['status'] = 'error'
        result['error'] = f"NewsAPI.ai API network error: {str(e)}"
        result['error_details'] = get_error_details(str(e), None)
    
    return result

def fetch_gnews_usage(api_key, days=30):
    """Fetch usage data from GNews API"""
    result = {'provider': 'gnews'}
    key_preview = f"{api_key[:15]}...{api_key[-15:]}" if len(api_key) > 30 else "SHORT_KEY"
    print(f"Testing GNews key: {key_preview}")
    
    try:
        print(f"Testing GNews API access...")
        test_response = requests.get(
        ...
        print(f"GNews API response: {test_response.status_code}")
        if test_response.status_code == 200:
            print(f"✓ GNews API access works!")
            result['status'] = 'success'
            result['basic_access'] = True
            result['message'] = 'GNews API key is working'
            
            response_data = test_response.json()
            if 'totalArticles' in response_data:
                result['sample_data'] = {
                    'total_articles': response_data.get('totalArticles', 0),
                    'articles_returned': len(response_data.get('articles', []))
                }
        else:
            print(f"✗ GNews API failed: {test_response.status_code}")
            result['status'] = 'error'
            result['error'] = f"GNews API failed: {test_response.status_code} - {test_response.text}"
            result['error_details'] = get_error_details(test_response.text, test_response.status_code)
            
    except requests.exceptions.RequestException as e:
        print(f"✗ GNews API network error: {str(e)}")
        result['status'] = 'error'
        result['error'] = f"GNews API network error: {str(e)}"
        result['error_details'] = get_error_details(str(e), None)
    
    return result

def fetch_serpapi_usage(api_key, days=30):
    """Fetch usage data from SerpAPI"""
    result = {'provider': 'serpapi'}
    key_preview = f"{api_key[:15]}...{api_key[-15:]}" if len(api_key) > 30 else "SHORT_KEY"
    print(f"Testing SerpAPI key: {key_preview}")
    
    try:
        print(f"Testing SerpAPI access...")
        test_response = requests.get(
        ...
        print(f"SerpAPI response: {test_response.status_code}")
        if test_response.status_code == 200:
            print(f"✓ SerpAPI access works!")
            result['status'] = 'success'
            result['basic_access'] = True
            result['message'] = 'SerpAPI key is working'
            
            response_data = test_response.json()
            if 'search_metadata' in response_data:
                result['sample_data'] = {
                    'search_results': len(response_data.get('organic_results', [])),
                    'total_time': response_data.get('search_metadata', {}).get('total_time_taken', 0)
                }
        else:
            print(f"✗ SerpAPI failed: {test_response.status_code}")
            result['status'] = 'error'
            result['error'] = f"SerpAPI failed: {test_response.status_code} - {test_response.text}"
            result['error_details'] = get_error_details(test_response.text, test_response.status_code)
            
    except requests.exceptions.RequestException as e:
        print(f"✗ SerpAPI network error: {str(e)}")
        result['status'] = 'error'
        result['error'] = f"SerpAPI network error: {str(e)}"
        result['error_details'] = get_error_details(str(e), None)
    
    return result

def fetch_google_cse_usage(api_key, days=30):
    """Fetch usage data from Google Custom Search Engine"""
    result = {'provider': 'google_cse'}
    key_preview = f"{api_key[:15]}...{api_key[-15:]}" if len(api_key) > 30 else "SHORT_KEY"
    print(f"Testing Google CSE ID: {key_preview}")
    
    # Note: Google CSE requires both API key and CSE ID
    # For testing, we'll just validate the format
    try:
        result['status'] = 'info_only'
        result['message'] = 'Google CSE ID stored (requires API key for testing)'
        result['note'] = 'Google Custom Search requires both CSE ID and API key. Add your Google API key separately.'
        
    except Exception as e:
        result['status'] = 'error'
        result['error'] = f"Google CSE validation error: {str(e)}"
        result['error_details'] = get_error_details(str(e), None)
    
    return result

def fetch_aws_usage(access_key, days=30):
    """Fetch usage data from AWS Cost Explorer API"""
    result = {'provider': 'aws'}
    key_preview = f"{access_key[:15]}...{access_key[-15:]}" if len(access_key) > 30 else "SHORT_KEY"
    print(f"Testing AWS access key: {key_preview}")
    
    try:
        # Note: This requires AWS SDK (boto3) to be installed
        # For now, we'll do basic validation and show info
        result['status'] = 'info_only'
        result['message'] = 'AWS Access Key stored (requires Secret Key and boto3 for full testing)'
        result['note'] = 'AWS Cost Explorer requires both Access Key and Secret Access Key. Install boto3 and add secret key for full functionality.'
        
        # Basic format validation
        if access_key.startswith('AKIA') or access_key.startswith('ASIA'):
            result['sample_data'] = {
                'key_type': 'Long-term' if access_key.startswith('AKIA') else 'Temporary',
                'key_format': 'Valid AWS Access Key format'
            }
        else:
            result['status'] = 'error'
        ...
        result['status'] = 'error'
        result['error'] = f"AWS validation error: {str(e)}"
        result['error_details'] = get_error_details(str(e), None)
...
def fetch_groq_usage(api_key, days=30):
    ...
    key_preview = f"{api_key[:15]}...{api_key[-15:]}" if len(api_key) > 30 else "SHORT_KEY"
    print(f"Testing Groq key: {key_preview}")
    
    try:
        print(f"Testing Groq API access...")
        test_response = requests.post(
        ...
        print(f"Groq API response: {test_response.status_code}")
        if test_response.status_code == 200:
            print(f"✓ Groq API access works!")
            result['status'] = 'success'
            result['basic_access'] = True
            result['message'] = 'Groq API key is working'
            ...
            print(f"✗ Groq API failed: {test_response.status_code}")
            result['status'] = 'error'
            result['error'] = f"Groq API failed: {test_response.status_code} - {test_response.text}"
            result['error_details'] = get_error_details(test_response.text, test_response.status_code)
            
    except requests.exceptions.RequestException as e:
        print(f"✗ Groq API network error: {str(e)}")
        result['status'] = 'error'
        result['error'] = f"Groq API network error: {str(e)}"
        result['error_details'] = get_error_details(str(e), None)
...
def fetch_api_usage(api_key, days=30):
    ...
    if provider == 'openai':
        ...
        return fetch_newsdata_usage(api_key, days)
    elif provider == 'newsapi_org':
        return fetch_newsapi_org_usage(api_key, days)
    elif provider == 'newsapi_ai':
        return fetch_newsapi_ai_usage(api_key, days)
    elif provider == 'gnews':
        return fetch_gnews_usage(api_key, days)
    elif provider == 'serpapi':
        return fetch_serpapi_usage(api_key, days)
    elif provider == 'google_cse':
        return fetch_google_cse_usage(api_key, days)
    elif provider == 'groq':
        return fetch_groq_usage(api_key, days)
    elif provider == 'aws':
        return fetch_aws_usage(api_key, days)
    else:
        return {
            'status': 'unsupported',
...
def add_key_endpoint():
    ...
    if not is_valid_api_key_format(full_key):
        return jsonify({'error': 'Invalid API key format - see supported formats in placeholder text'}), 400
...
def test_key_endpoint(key_id):
    ...
    try:
        ...
        if provider == 'openai':
            ...
            # Test NewsData.io key (corrected implementation)
            response = requests.get(
            ...
            if response.status_code == 200:
                return jsonify({
                    'status': 'success',
                    'message': 'NewsData.io API key is valid and working'
                })
            else:
                return jsonify({
                    'status': 'error',
                    'message': f'NewsData.io API key test failed: {response.status_code}'
                }), 400
            ...
            if response.status_code == 200:
                return jsonify({
                    'status': 'success',
                    'message': 'NewsAPI.org API key is valid and working'
                })
            else:
                return jsonify({
                    'status': 'error',
                    'message': f'NewsAPI.org API key test failed: {response.status_code}'
                }), 400
                
        elif provider == 'newsapi_ai':
            # Test NewsAPI.ai key (corrected implementation)
            response = requests.get(
                'https://eventregistry.org/api/v1/article/getArticles',
                params={
                    'resultType': 'articles',
            ...
            if response.status_code == 200:
                return jsonify({
                    'status': 'success',
                    'message': 'NewsAPI.ai API key is valid and working'
                })
            else:
                return jsonify({
                    'status': 'error',
                    'message': f'NewsAPI.ai API key test failed: {response.status_code}'
                }), 400
            ...
            if response.status_code == 200:
                return jsonify({
                    'status': 'success',
                    'message': 'GNews API key is valid and working'
                })
            else:
                return jsonify({
                    'status': 'error',
                    'message': f'GNews API key test failed: {response.status_code}'
                }), 400
            ...
            if response.status_code == 200:
                return jsonify({
                    'status': 'success',
                    'message': 'SerpAPI key is valid and working'
                })
            else:
                return jsonify({
                    'status': 'error',
                    'message': f'SerpAPI key test failed: {response.status_code}'
                }), 400
                
        elif provider == 'google_cse':
            # Google CSE - just validate format
            return jsonify({
                'status': 'success',
                'message': 'Google CSE ID format is valid (requires API key for full testing)'
            })
            
        elif provider == 'aws':
            # AWS Access Key - just validate format
            return jsonify({
                'status': 'success',
                'message': 'AWS Access Key format is valid (requires Secret Access Key for full testing)'
            })
            ...
            if response.status_code == 200:
                ...
                return jsonify({
                    'status': 'error',
                    'message': f'Groq API key test failed: {response.status_code}'
                }), 400
        else:
            return jsonify({
                'status': 'error',
...
if __name__ == '__main__':
    ...
    app.run(debug=True, host='0.0.0.0', port=5000), api_key, re.IGNORECASE):
        return True
    # UUID format (NewsAPI.ai)
...
def fetch_newsdata_usage(api_key, days=30):
    ...
    try:
        print(f"Testing NewsData.io API access...")
        # Corrected: Use apikey parameter, not X-ACCESS-KEY header
        test_response = requests.get(
            'https://newsdata.io/api/1/latest',
            params={
                'apikey': api_key,
...
def fetch_newsapi_ai_usage(api_key, days=30):
    """Fetch usage data from NewsAPI.ai"""
    headers = {
        'Authorization': f'Bearer {api_key}'
    }
    ...
    try:
        ...
        if provider == 'openai':
            ...
            response = requests.get(
                'https://api.newsapi.ai/api/v1/article/getArticles',
                headers=headers,
                params={'query': 'test', 'resultsCount': 1},
                timeout=10
            )
